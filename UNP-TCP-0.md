...
5. **四次挥手可能出现的异常：**
   ...
   4. 第四次挥手丢包，客户端进入TIME_WAIT状态，等待2MSL，服务器由于收不到LAST-ACK则进行重发，如果多次重发失败，则服务端连接超时关闭；TIME_WAIT状态可以通过设置内核参数，取消TIME_WAIT状态，让端口可以立刻复用，同时为了防止上一个连接的错误信息，内核会根据`tcp`报文的`seq`判断是否本次连接的报文，不是的话会给对端发送`RST`
